...
### Crosswalk?
Crosswalks = «Zebrastreifen», Konvertierung von einem Metadatenstandard in einen anderen. Gängiger Begriff, um die Konvertierung von einem Metadatenstandard in einen anderen zu beschreiben. Der “Crosswalk” beinhaltet Regeln wie Elemente und Werte zugeord-net/verändert werden müssen (sog. Mapping). Im Idealfall verlustfrei, aber meist keine 1:1-Zuordnung möglich. Beispiel: MARC21 zu Dublin Core.
...
Zeigt Archiv-/Bibliotheksinformationsdatenbanken (Koha, ArchivesSpace, DSpace) via Schnittstelle (OAI-PMH) Metadaten mit VuFindHarvest Datenformate MARC21-XML, EAD, DC  abspeichern.
Schritt 1: VuFind greift via Schnittstelle auf Datenbank zu, holt Metadaten und speichert diese im gewünschten Format ab.
Schritt 2: Verschiedene Datenformate einheitlich in MARC21-XML konvertieren.
Fazit:
EAD muss zuerst in MARC (ist ein Binärformat), dann in einem zweiten Schritt in MARCXML konvertiert werden.
Im Header kann im Namespace abgelesen werden, in welchem Metadatenformat die Daten angelegt sind.
Aus Koha konnten die Daten direkt in MARCXML «geerntet» werden und müssen deshalb natürlich nicht weiter behandelt werden. 
